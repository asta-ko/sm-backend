...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            def court_result(self, decision_text):  # убрать temp_table после отладки
                ...
                if 'штраф' in decision_text:
                    ...
                    if fine_num or fine_hidden:
                        result['fine'] = {'fine_num': fine_num, 'fine_hidden': fine_hidden}
                ...
                if fine_not_found and 'арест' in decision_text:
                    ...
                    if arrest_days or arrest_hidden:
                        result['arrest'] = {'days': arrest_days, 'hidden': arrest_hidden}
                ...
                if fine_not_found and arrest_not_found and (
                    ...
                    if works_hours or works_hidden:
                        result['works'] = {'hours': works_hours, 'hidden': works_hidden}
        ...
        cases = Case.objects.filter(type=2, stage=1, result_text__isnull=False)[:10]
        pe = KoapPenaltyExtractor()
        # for x in ['одна тысяча пятьсот', 'две тысячи пятьсот', 'четыре тысячи пятьсот']:
        #     print(pe.get_num_from_text(x))
        ...
        for x in cases:
            print(x.result_text)
            # result = pe.court_result(x.result_text)
            # if result.get('could_not_process'):
            #     not_processed += 1
        print(not_processed)
