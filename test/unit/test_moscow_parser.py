...
def test_save_case(moscow_courts, koap_articles):
    ...
    cases_urls = ['https://mos-gorsud.ru/rs/meshchanskij/services/cases/admin/details/e759cbf7-cacd-4642-91c1-7a8479cfe270?documentMainArticle=%D1%81%D1%82.+19.34%2C+%D0%A7.2&formType=fullForm',
    ...
    MoscowParser(url=url, stage=1).save_cases(urls=cases_urls)
...
def test_urls_getter():
    ...
    urls = MoscowParser(url=url, stage=1).get_all_cases_urls(limit_pages=True)
    print(urls)
    assert len(urls)
